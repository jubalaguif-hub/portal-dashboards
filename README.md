...
- **📂 Flexibilidade**: Criar e organizar categorias conforme necessidade

### 🏥 Contexto Institucional

Desenvolvido para suportar as operações diárias do hospital, o portal organiza recursos em:

- **🩺 Assistencial**: Indicadores de qualidade, produtividade e performance médica (padrão)
- **👥 RH/PJ**: Controle de pessoal, folha de pagamento e gestão de colaboradores (padrão)
- **📑 Planilhas**: Recursos administrativos, operacionais e de controle (padrão)
- **📂 Categorias Customizadas**: Personalizáveis conforme necessidade da instituição (novo!)
...
- **Criar novas categorias** além das 3 pré-definidas (Assistencial, RH, Planilhas)
- **Ícones flexíveis**: Emoji + Texto ou Imagem (PNG/JPG)
- **Redimensionamento automático** de imagens para 24x24px
- **Editar e deletar categorias** dinamicamente
- **Persistência em localStorage** com sincronização automática
- **Organização dinâmica** de planilhas por categoria
...
- **Suporte a categorias customizadas** na busca
...
1. **Carregamento Inicial**
   ...
   - localStorage carrega preferências

2. **Inicialização**
   - Verificação de tema salvo
   - Carregamento de categorias customizadas
   - Carregamento de planilhas customizadas
...
3. **Interação do Usuário**
   ...
   - Categorias → CRUD com localStorage

4. **Estado da Aplicação**
   - **Seções**: Apenas uma ativa por vez
   - **Busca**: Modo global vs. seção ativa
   - **Tema**: Aplicado via CSS variables
   - **Categorias**: Customizadas + padrão (persistidas)
   - **Planilhas**: Organizadas por categoria

### 📊 Estados da Aplicação

| Estado | Descrição | Comportamento |
|--------|-----------|---------------|
| **Inicial** | Primeira carga | Carrega seção padrão + tema salvo + categorias |
...
| **Categoria Criada** | Nova categoria | Menu item + seção adicionados dinamicamente |
...
// Categorias Customizadas
{
  "customCategories": [
    {
      "id": "cat-1706391500000",
      "name": "Financeiro",
      "icon": "💰",
      "iconImage": "data:image/png;base64,...", // ou null
...
// Planilhas Customizadas
{
  "customSheets": [
    {
      "id": "custom-1706391500000",
      "name": "Orçamento",
      "link": "https://...",
      "category": "cat-1706391500000",  // ID da categoria
...
├── ⚙️ script.js                  # Lógica e interações (com CRUD de categorias)
├── 📖 README.md                  # Esta documentação
├── 📝 MUDANCAS_CATEGORIA_IMAGEM.md # Changelog do sistema de categorias
│
...
| Arquivo | Função | Tamanho | Tipo | Notas |
|---------|--------|---------|------|-------|
| `index.html` | Estrutura da página | ~25KB | HTML5 | Inclui modais para categorias |
| `style.css` | Estilos e temas | ~45KB | CSS3 | Suporte a ícones emoji e imagem |
| `script.js` | Interatividade | ~45KB | JavaScript | CRUD completo de categorias |
| `MUDANCAS_CATEGORIA_IMAGEM.md` | Changelog | ~5KB | Markdown | Detalhes do sistema de ícones |
| Assets | Imagens otimizadas | ~500KB | WebP/PNG | Categorias customizadas podem incluir imagens |
...
1. **Acesse o menu "Gerenciar"** → "Categoria" → "Criar Categoria"
2. **Preencha o nome** (obrigatório) - Ex: "Financeiro"
3. **Escolha um ícone** (opcional):
   - Emoji/Texto - Ex: 💰, 📊, 🏥
   - OU Imagem PNG/JPG (redimensiona automaticamente para 24x24px)
4. **Clique em "Criar Categoria"**
5. A categoria aparece automaticamente:
   - No menu lateral (à esquerda)
   - Como nova seção no dashboard
   - Disponível para organizar planilhas

### 📋 Gerenciar Categorias

**Editar Categoria:**
1. Menu "Gerenciar" → "Categoria" → "Editar Categoria"
2. Selecione a categoria desejada
3. Altere nome e/ou ícone
4. Clique "Salvar"

**Deletar Categoria:**
1. Menu "Gerenciar" → "Categoria" → "Deletar Categoria"
2. Selecione a categoria
3. Confirme exclusão (planilhas serão movidas para "Planilhas")
...
### 🎨 Customizar Ícones de Categoria

**Tamanhos Padrão:**
- Menu lateral: 24x24px
- Título de seção: 28x24px
- Preview em edição: 32x32px

**Formatos Suportados:**
- Emoji: 💰, 📊, 🏥, etc.
- Texto: Qualquer texto com até 5 caracteres
- Imagem: PNG ou JPG (redimensionadas automaticamente)
...
|--------|----------|-------------|
| `handleAddCategory()` | Criar categoria | script.js ~1340 |
| `createCustomCategorySection()` | Gerar seção | script.js ~1446 |
| `addCustomCategoryToMenu()` | Gerar menu item | script.js ~1475 |
| `openEditCategoryModal()` | Abrir edição | script.js ~1538 |
| `showEditCategoryForm()` | Formulário edição | script.js ~1603 |
| `getAllCategories()` | Carregar todas | script.js ~1366 |
| `loadCustomCategories()` | Carregar localStorage | script.js ~1395 |
| `handleSearch()` | Busca global | script.js |
| `showSection()` | Navegar seção | script.js |
| `toggleTheme()` | Alternar tema | script.js |
...
- [ ] Criar categoria com emoji
- [ ] Criar categoria com imagem
- [ ] Editar categoria (alterar ícone)
- [ ] Deletar categoria
- [ ] Adicionar planilha a categoria customizada
- [ ] Recarregar página (dados persistem)
- [ ] Buscar em categoria customizada
- [ ] Mover planilha entre categorias
...
**🚀 Pronto para uso em produção, evolução contínua e categorias customizáveis!**

---

*Última atualização: Janeiro 2026*  
*Versão: 2.1* ✨ **Nova versão com Sistema de Categorias Customizáveis**  
